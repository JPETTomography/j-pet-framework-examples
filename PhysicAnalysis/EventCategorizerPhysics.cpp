...
bool EventCategorizerPhysics::init()
{
  ...
  if (fSaveControlHistos) {
    ...
    // Stream specyfic histograms
    getStatistics().createHistogram(
      new TH1F("TOT", "TOT of Hits", 1000, -0.5, 99.5)
    );
    getStatistics().getHisto1D("TOT")->SetXTitle("TOT [ns]");
    getStatistics().getHisto1D("TOT")->SetYTitle("Counts");

    getStatistics().createHistogram(
		  new TH1F("Positronium_Lifetime",
    ...
    getStatistics().createHistogram(
		  new TH1F("DecayInto2_DLOR", "DLOR distance", 500, -0.5, 49.5)
    );
    ...
    getStatistics().createHistogram(
		  new TH1F("Annihilation_Hits_in_event", "Number of Annihilation Hits in Event", 50, -0.5, 49.5)
    );
    ...
    getStatistics().createHistogram(
		  new TH1F("Deexcitation_Hits_in_event", "Number of Deexcitation Hits in Event", 50, -0.5, 49.5)
    );
...
JPetEvent EventCategorizerPhysics::physicsAnalysis(vector<JPetHit> hits)
{
  ...
  for (unsigned i=0; i<hits.size(); i++ ) {
    if (fabs(hits[i].getPosZ()) < fMaxZPos)	{
      ...
      if (fSaveControlHistos) { getStatistics().getHisto1D("TOT")->Fill(TOTofHit/1000.); }
  ...
  if (deexcitationHits.getHits().size() > 0) {
    ...
    if (annihilationHits.getHits().size() > 0)	{
      if (fSaveControlHistos) {
        getStatistics().getHisto1D("Positronium_Lifetime")
          ->Fill( annihilationHits.getHits().at(0).getTime()/1000. - deexcitationHits.getHits().at(0).getTime()/1000.);
      }
	  }
  }
