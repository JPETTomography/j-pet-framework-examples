...
///////////////////////////////////////////////////////////////////////////////
	getStatistics().createHistogram(
			new TH2F("hit_oposite_time_diff_layer1",
								"Two Oposite Hit time difference - Layer1",
								24, 0.5, 24.5,
								120, -6000., 6000.)
		);
...
getStatistics().createHistogram(
			new TH2F("hit_oposite_time_diff_layer2",
								"Two Oposite Hit time difference - Layer2",
								24, 0.5, 24.5,
								120, -6000., 6000.)
		);
...
getStatistics().createHistogram(
			new TH2F("hit_oposite_time_diff_layer3",
								"Two Oposite Hit time difference - Layer3",
								48, 0.5, 48.5,
								120, -6000., 6000.)
		);
...
void EventCategorizer::exec(){
	...
	if(auto event = dynamic_cast<const JPetEvent*const>(getEvent())){
		...
		if(event->getHits().size()==2){ //2 hits in one event
	...
	int slot_number_second = secondHit.getBarrelSlot().getID();
	    ...
	    float timeDiff = firstHit.getTime()-secondHit.getTime();
...
//cout<<"slot_first="<<slot_number_first<<endl;
	//cout<<"slot_second="<<slot_number_second<<endl;

	if(slot_number_first<25 && slot_number_second==slot_number_first+24){

	slot_no=slot_number_first;

	 getStatistics().getHisto2D("hit_oposite_time_diff_layer1")
                  .Fill(slot_no,timeDiff);

	}

	if(slot_number_first>24 && slot_number_second==slot_number_first-24){
...
if(firstHit.getBarrelSlot().getLayer().getID()==2 && secondHit.getBarrelSlot().getLayer().getID()==2){
...
//cout<<"slot_first="<<slot_number_first<<endl;
	...
	getStatistics().getHisto2D("hit_oposite_time_diff_layer2")
                  .Fill(slot_no,timeDiff);
...
if(firstHit.getBarrelSlot().getLayer().getID()==3 && secondHit.getBarrelSlot().getLayer().getID()==3){
	...
	slot_no=slot_number_first-96;
 	...
 	getStatistics().getHisto2D("hit_oposite_time_diff_layer3")
                  .Fill(slot_no,timeDiff);
