...
void SinogramCreatorTools::swapIfNeeded(float& firstX, float& firstY, float& firstT, float& secondX, float& secondY, float& secondT) {
  if (firstX > secondX) {
...
int SinogramCreatorTools::getTOFSlice(float firstX, float firstY, float firstTOF, float secondX, float secondY, float secondTOF, float sliceSize) {
  swapIfNeeded(firstX, firstY, firstTOF, secondX, secondY, secondTOF);
