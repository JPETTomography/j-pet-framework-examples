...
void JPetRecoSignalTools::savePNGOfBadSignal(const JPetRecoSignal &signal,
                                             int numberOfBadSignals) {
  TCanvas *c1 = new TCanvas();
  TGraph *badSignal = JPetRecoSignalTools::plotJPetRecoSignal(signal);
  badSignal->Draw("AP");
  std::string title;
...
void JPetRecoSignalTools::savePNGwithMarkedOffsetsAndStartingPoints(
    const JPetRecoSignal &signal, int number) {
  TCanvas *c1 = new TCanvas();
  TGraph *badSignal = JPetRecoSignalTools::plotJPetRecoSignal(signal);
  badSignal->Draw("AP");
  badSignal->Draw("ALP");
  ...
  TLine *offsetLine =
      new TLine(signalPoints[0].time / 1000, offset - 100,
                signal.getRecoTimeAtThreshold(-100) / 1000, offset - 100);
  std::cout << "Time at thr: " << signal.getRecoTimeAtThreshold(-100) / 1000
            << std::endl;
...
double
JPetRecoSignalTools::calculateTimeAtThreshold(const JPetRecoSignal &signal,
...
double
JPetRecoSignalTools::calculateConstantThreshold(const JPetRecoSignal &signal,
                                                const double threshold) {
  const std::vector<shapePoint> signalPoints = signal.getShape();
  ...
  double slope = 1;
  double intercept = 0;
  for (int i = 0; i < findIndexAtValue(min(amplitudePoints), amplitudePoints);
       i++) {

    if (amplitudePoints[i + 1] < thresholdPlusOffset &&
        amplitudePoints[i] > thresholdPlusOffset) {
      slope = (amplitudePoints[i + 1] - amplitudePoints[i]) /
              (timePoints[i + 1] - timePoints[i]);
      intercept = amplitudePoints[i] -
                  (amplitudePoints[i + 1] - amplitudePoints[i]) /
                      (timePoints[i + 1] - timePoints[i]) * timePoints[i];
...
double
JPetRecoSignalTools::calculateConstantFraction(const JPetRecoSignal &signal,
                                               const double threshold) {
...
double JPetRecoSignalTools::calculateAreaFromStartingIndex(
    const JPetRecoSignal &signal) {
  if (signal.getOffset() == JPetRecoSignalTools::ERRORS::badOffset) {
  ...
  for (unsigned int i = 0; i < timePoints.size() - 1; i++) {
    if ((amplitudePoints[i] > 0 && amplitudePoints[i + 1] < 0) ||
        (amplitudePoints[i] < 0 && amplitudePoints[i + 1] > 0)) {
      double xZero = pktPrzecieciaOX(timePoints[i], amplitudePoints[i],
                                     timePoints[i + 1], amplitudePoints[i + 1]);
      if (amplitudePoints[i] > 0 && amplitudePoints[i + 1] < 0) {
        area = area + 0.5 * (xZero - timePoints[i]) * amplitudePoints[i] +
               0.5 * (timePoints[i + 1] - xZero) * amplitudePoints[i + 1];
      }
      if (amplitudePoints[i] < 0 && amplitudePoints[i + 1] > 0) {
        area = area + 0.5 * (xZero - timePoints[i]) * amplitudePoints[i] +
               0.5 * (timePoints[i + 1] - xZero) * amplitudePoints[i + 1];
      ...
      if (amplitudePoints[i] < amplitudePoints[i + 1]) { // different slopes
        area = area + amplitudePoints[i] * (timePoints[i + 1] - timePoints[i]) +
               0.5 * (amplitudePoints[i + 1] - amplitudePoints[i]) *
                   (timePoints[i + 1] - timePoints[i]);
      } else {
        area = area +
               amplitudePoints[i + 1] * (timePoints[i + 1] - timePoints[i]) +
               0.5 * (amplitudePoints[i] - amplitudePoints[i + 1]) *
                   (timePoints[i + 1] - timePoints[i]);
      }
    }
  }
  ...
  area = area / resistance /
         1000; // 50 ohms resistance and units change to pC from m * p t
...
double JPetRecoSignalTools::calculateArea(const JPetRecoSignal &signal) {
  ...
  if (timePoints[timePoints.size() - 1] < 0 && timePoints[0] > 0)
    range = timePoints[0] - timePoints[timePoints.size() - 1];
  else
    range = (timePoints[timePoints.size() - 1] - timePoints[0]);
...
int JPetRecoSignalTools::findStartingIndex(const JPetRecoSignal &signal) {
  ...
  int indexAtMinimumOfSignal =
      findIndexAtValue(min(amplitudePoints), amplitudePoints);

  // Calculating approximate mean and std deviation of noise, estimating on
  // first 20 points
  const int numberOfPointsTakenForAproximation = 20;
  double first20PointsMean = calculateArithmeticMean(
      amplitudePoints, numberOfPointsTakenForAproximation);
  double first20PointsDeviation = calculateStandardDeviation(
      amplitudePoints, numberOfPointsTakenForAproximation);

  // Checking if index was calculated correctly

  if (indexAtMinimumOfSignal ==
      JPetRecoSignalTools::ERRORS::badIndexAtMinimumValue) {
    std::cout << "Problem with finding proper index at findStartingIndex\n";
    return JPetRecoSignalTools::ERRORS::badStartingIndex;
  }

  // Checking if minimum was not in first 20 points, which means that signal was
  // not aquisited properly
  ...
  amplitudePoints =
      copyVectorWithNumbersUpToIndex(amplitudePoints, indexAtMinimumOfSignal);

  // Start searching for last point in noise, go point by point to left from
  // minium on signal as long as noise will be found

  int index = indexAtMinimumOfSignal;

  while (index > numberOfPointsTakenForAproximation) {
    if (isPointFromRecoSignalInNoise(first20PointsMean, first20PointsDeviation,
                                     amplitudePoints[index])) {
      return index;
    }
    index -= 1;
  }
...
double JPetRecoSignalTools::calculateOffset(const JPetRecoSignal &signal) {
  ...
  // Finding place where minimum of signal is
  const int indexAtMinimumOfSignal =
      findIndexAtValue(min(amplitudePoints), amplitudePoints);
  ...
  // Calculating approximate mean and std deviation of noise, estimating on
  // numberOfPointsTakenForAproximation points
  double first20PointsMean = calculateArithmeticMean(
      amplitudePoints, numberOfPointsTakenForAproximation);
  double first20PointsDeviation = calculateStandardDeviation(
      amplitudePoints, numberOfPointsTakenForAproximation);
  ...
  while (index > numberOfPointsTakenForAproximation) {
    if (isPointFromRecoSignalInNoise(first20PointsMean, first20PointsDeviation,
                                     amplitudePoints[index])) {
      return calculateArithmeticMean(amplitudePoints,
                                     index); // HERE CORRECT ARITHEMTIC MEAN
    }
    index -= 1;
  }
...
bool JPetRecoSignalTools::isPointFromRecoSignalInNoise(
    const double noiseMean, const double noiseDeviation, const double point) {
  const double topBorder = noiseMean + 3 * noiseDeviation,
               bottomBorder = noiseMean - 3 * noiseDeviation;

  return ((point < topBorder) && (point > bottomBorder));
}

double JPetRecoSignalTools::calculateStandardDeviation(
    const std::vector<double> &vector, const int upToIndex) {
  if (vector.size() == 1) {
...
std::vector<double>
JPetRecoSignalTools::copyVectorWithNumbersUpToIndex(std::vector<double> &vector,
                                                    int index) {
...
double JPetRecoSignalTools::calculateArithmeticMean(
...
double
JPetRecoSignalTools::calculateArithmeticMean(const std::vector<double> &vector,
                                             const int upToIndex) {
  double mean = 0;
  for (int index = 0; index < upToIndex + 1; ++index) {
    mean += vector[index];
...
double JPetRecoSignalTools::pktPrzecieciaOX(double x1, double y1, double x2,
                                            double y2) {
  double slope;
  double intercept;
  slope = (y1 - y2) / (x1 - x2);
...
int JPetRecoSignalTools::findIndexAtValue(double value,
...
TGraph *JPetRecoSignalTools::plotJPetRecoSignal(const JPetRecoSignal &signal) {
  ...
  TGraph *gr =
      new TGraph(signalPoints.size(), &timePoints[0], &amplitudePoints[0]);
  gr->SetTitle("");
...
void JPetRecoSignalTools::saveTH1FsToRootFile(
    std::vector<TH1F *> histoCollection, std::string fileName,
    std::string pdfName) {
  /*	Saves histos in vector to rootfile of name fileName.root in
   * directoryName assumes that histograms are given by pointers and are stored
   * in vector saves also a Canvas with all histograms, assumes that first
   * histogram in a collection is base one, also saves pdfs... this function is
   * bad as hell!
   */
...
int JPetRecoSignalTools::findIndexOfMaxHisto(
    std::vector<TH1F *> histoCollection) {
  int max = -100000.;
  int histoMax = 0;
  int maxIndex = histoCollection.size() + 5;

  for (unsigned int histoNumber = 0; histoNumber < histoCollection.size();
       histoNumber++) {
    histoMax = histoCollection[histoNumber]->GetBinContent(
        histoCollection[histoNumber]->GetMaximumBin());
    if (histoMax > max) {
      max = histoMax;
