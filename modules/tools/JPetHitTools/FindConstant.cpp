...
FindConstant::FindConstant(const std::vector<double> &inputEvents,
                           const TString file, const double sourcePos,
                           const int scinID) {
  ...
  EXPHisto = new TH1F("Experimental charge histogram", "expHisto", 500 / 2,
                      minBin, maxBin);
  SIMHisto = new TH1F("Simulated energy histogram", "simHisto", 500 / 2, minBin,
                      maxBin);
  ...
  bestChi2 = 999999, bestNorm = 1, bestAlpha = 1, alpha = 1, normalisation = 1,
  bestNumberOfBins = 0;
...
double FindConstant::execute() {
  ...
  for (double eRes = 1.0; eRes < 2.0; eRes += 0.05) {
    ...
    numberOfBins =
        -1.0 * (double(SIMHisto->GetXaxis()->FindBin(lowerCut / alpha)) -
                double(SIMHisto->GetXaxis()->FindBin(upperCut / alpha)));
    ...
    INFO(Form(
        "Beta equal to %f was fitted with chi2: %f for number of bins: %d",
        eRes, compareHistogramsByChi2(normalisation, alpha), numberOfBins));
...
void FindConstant::fillEXPHisto(const double normalisation,
                                const double alpha) {
  EXPHisto->Reset();
...
void FindConstant::produceSIMEvents(std::vector<double> &SIMEvents,
                                    const double eRes) {
  ...
  for (i = 0; i < 100000;) {
    ...
    przekroj = 2.0 * 3.14 * stala * pow(Eprim / E, 2.0) *
               (E / Eprim + Eprim / E - 1.0 + pow(kos, 2.0)) *
               ((511.0 / E + 1.0) / (T - E) -
                (511.0 / E * T + T - E) / (pow((T - E), 2.0))) *
               (-1.0) * 1E27;
...
void FindConstant::fillSIMHisto(const double normalisation,
                                const double alpha) {
  SIMHisto->Reset();
...
void FindConstant::aproximateParameters() {
  if (NULL == EXPHisto && NULL == SIMHisto) {
    ERROR(Form(
        "Not initialised histograms in findEnergyConstant! Exiting program"));
    std::exit(1);
  }
...
double FindConstant::estimateWidthRatio() {
  ...
  initialWidthRatio =
      ((double)SIMHisto->GetBinCenter((SIMHisto->GetMaximumBin()))) /
...
double FindConstant::estimateHeightRatio() {
  ...
  heighestCountsInSIM =
      (double)SIMHisto->GetBinContent((SIMHisto->GetMaximumBin()));
  heighestCountsInEXP =
      ((double)EXPHisto->GetBinContent(EXPHisto->GetMaximumBin()));
...
void FindConstant::performFit() {
  ...
  for (double currentAlpha = 0.8 * alpha; currentAlpha < 1.2 * alpha;
       currentAlpha += alpha * 0.01) {
    for (double currentNorm = 0.8 * normalisation;
         currentNorm < 1.2 * normalisation;
         currentNorm += normalisation * 0.01) {
...
double FindConstant::compareHistogramsByChi2(const double normalisation,
                                             const double alpha) {
  double chi2 = 0.0;
  ...
  for (unsigned int i = 0; i < EXPHisto->GetSize() - (unsigned int)2; i++) {
    ...
    if (double(SIMHisto->GetBinCenter(i)) > lowerCut / alpha &&
        double(SIMHisto->GetBinCenter(i)) < upperCut / alpha) {
      chi2 += pow((double(SIMHisto->GetBinContent(i)) -
                   double(EXPHisto->GetBinContent(i))),
                  2.0) /
              (double(EXPHisto->GetBinContent(i)) +
               (double(SIMHisto->GetBinContent(i)) / normalisation));
    }
  }
  return chi2;
}
...
void FindConstant::saveFitResultToTxt(std::string name) {
  ...
  if (myfile.is_open()) {
    myfile << "Final fit result for data:" << filePath
           << " for scintillator: " << scintillatorID << " for position "
           << sourcePosition << "   alpha: " << bestAlpha
           << " and beta: " << energyResolution << " and chi2: " << bestChi2
           << " for " << bestNumberOfBins << " bins " << std::endl;
  }

  myfile.close();
}

void FindConstant::drawChi2AndFitPol2(const std::vector<double> &res,
  ...
  quadraticFit = new TF1("quadraticFit", "[0]* (x - [1])**2 + [2]", ResToFit[0],
                         ResToFit[ResToFit.size() - 1]);
  ...
  std::stringstream buf;
  buf << sourcePosition;
  c1->SaveAs(("FitResults" + filePath + buf.str() +
              "/Chi2Plot_strip_fitRegion_" + streamForEResExtraction.str() +
              expHistoTitle + ".png"));
  ...
  c1->SaveAs(("FitResults" + filePath + buf.str() + "/Chi2Plot_strip_" +
              streamForEResExtraction.str() + expHistoTitle + ".png"));

  delete gr;
  delete c1;
}

double FindConstant::returnEnergyResolution() { return energyResolution; }
