...
bool SDARecoDrawAllOffsets::terminate() {
  ...
  int tallestHeight =
      (fOffsetHistos[0]->GetBinContent(fOffsetHistos[0]->GetMaximumBin()));
  for (unsigned int j = 1; j < fNumberOfPMTs; j++) {
    if (fOffsetHistos[j]->GetBinContent(fOffsetHistos[j]->GetMaximumBin()) >
        tallestHeight) {
      tallest = j;
      tallestHeight =
          fOffsetHistos[j]->GetBinContent(fOffsetHistos[j]->GetMaximumBin());
    }
  }
  ...
  legend->AddEntry(fOffsetHistos[tallest], fOffsetHistos[tallest]->GetTitle(),
                   "l");
