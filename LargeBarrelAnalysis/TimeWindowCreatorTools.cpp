...
{
  std::sort(input.begin(), input.end(), [](JPetSigCh sigCh1, JPetSigCh sigCh2) { return sigCh1.getValue() < sigCh2.getValue(); });
}

/**
 * Building all Signal Chnnels from one TDC
 */
vector<JPetSigCh> TimeWindowCreatorTools::buildSigChs(TDCChannel* tdcChannel, const JPetTOMBChannel& tombChannel,
                                                      map<unsigned int, vector<double>>& timeCalibrationMap,
                                                      map<unsigned int, vector<double>>& thresholdsMap, double maxTime, double minTime,
                                                      bool setTHRValuesFromChannels, JPetStatistics& stats, bool saveHistos)
{
  ...
  {
    // time data in TDCChannel objects from Unpacker library are in nanoseconds, converting to picoseconds
    auto leadTime = 1000.0 * tdcChannel->GetLeadTime(j);
    if (leadTime > maxTime || leadTime < minTime)
    {
      continue;
    }
    auto leadSigCh = generateSigCh(leadTime, tombChannel, timeCalibrationMap, thresholdsMap, JPetSigCh::Leading, setTHRValuesFromChannels);
    allTDCSigChs.push_back(leadSigCh);
    if (saveHistos)
    {
      stats.fillHistogram(Form("pm_occupation_thr%d", tombChannel.getLocalChannelNumber()), tombChannel.getPM().getID());
    }
  }
  ...
  {
    // time data in TDCChannel objects from Unpacker library are in nanoseconds, converting to picoseconds
    auto trailTime = 1000.0 * tdcChannel->GetTrailTime(j);
    if (trailTime > maxTime || trailTime < minTime)
    {
      continue;
    }
    auto trailSigCh = generateSigCh(trailTime, tombChannel, timeCalibrationMap, thresholdsMap, JPetSigCh::Trailing, setTHRValuesFromChannels);
 ...
 */
JPetSigCh TimeWindowCreatorTools::generateSigCh(double tdcChannelTime, const JPetTOMBChannel& channel,
                                                map<unsigned int, vector<double>>& timeCalibrationMap,
                                                map<unsigned int, vector<double>>& thresholdsMap, JPetSigCh::EdgeType edge,
                                                bool setTHRValuesFromChannels)
{
  JPetSigCh sigCh;
  // calibration constants are in nanoseconds, converting to picoseconds
  auto correction = 1000.0 * UniversalFileLoader::getConfigurationParameter(timeCalibrationMap, channel.getChannel());
  sigCh.setValue(tdcChannelTime + correction);
  sigCh.setType(edge);
  ...
  {
    sigCh.setThreshold(UniversalFileLoader::getConfigurationParameter(thresholdsMap, channel.getChannel()));
  }
  return sigCh;
}
