...
// the exec method collects JPetHit objects belonging to the same
// time window, puts them in the fHits vector and calls
// the fillCoincidenceHistos methods on all the hits from one
// time window
void TaskE::exec()
{
...
void TaskE::fillCoincidenceHistos(std::vector<JPetHit>& hits){
  ...
  for (auto i = hits.begin(); i != hits.end(); ++i) {
    for (auto j = i; ++j != hits.end(); /**/) {
      ...
      if (hit1.getBarrelSlot().getLayer() == hit2.getBarrelSlot().getLayer() &&
...
bool TaskE::isGoodTimeDiff(const JPetHit & hit, int thr){
  ...
  if( fabs( this_hit_timediff - mean_timediff ) < 1.0 ){
    return true;
  }
...
void TaskE::fillTOTvsTOThisto(int delta_ID, int thr, const JPetHit & hit1, const JPetHit & hit2){
  ...
  if( delta_ID != n_slots_in_half_layer ){
    return; // skip non-opposite coincidences
  }  
