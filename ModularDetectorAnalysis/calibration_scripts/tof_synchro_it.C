...
void tof_synchro_it(std::string fileName, std::string calibJSONFileName = "calibration_constants.json", bool saveResult = false,
                    std::string resultDir = "./", int iteration = 0, int minScinID = 201, int maxScinID = 512)
{
  ...
  if (fileHitsTOF->IsOpen())
  {
    ...
    TGraphErrors* peakDiffGraphZoom2 = new TGraphErrors();
    peakDiffGraphZoom2->SetNameTitle("peak_diff_graph_zoom_2", "Difference of peaks means vs. sci ID, smaler scale");
    peakDiffGraphZoom2->GetXaxis()->SetTitle("Scin ID");
    peakDiffGraphZoom2->GetYaxis()->SetTitle("time diff [ps]");
    peakDiffGraphZoom2->SetMinimum(-200.0);
    peakDiffGraphZoom2->SetMaximum(200.0);
    ...
    for (int scinID = minScinID; scinID <= maxScinID; ++scinID)
    {
      ...
      peakDiffGraphZoom2->SetPoint(graphIt, (double)scinID, tofCorr);
      peakDiffGraphZoom2->SetPointError(graphIt, 0.0, tofCorrError);
    ...
    if (saveResult)
    {
      ...
      TCanvas* canPeakDiffZoom2 = new TCanvas("peak_diff_scin_zoom_2", "peak_diff_scin_zoom_2", 1200, 800);
      string rmsTitle3 = "Iteration " + to_string(iteration) + ":   RMS_X=" + to_string(peakDiffGraphZoom2->GetRMS(1)) +
                         "   RMS_Y=" + to_string(peakDiffGraphZoom2->GetRMS(2));
      peakDiffGraphZoom2->SetTitle(rmsTitle3.c_str());
      peakDiffGraphZoom2->Draw("AP*");
      canPeakDiffZoom2->SaveAs(Form("%s/peak_diff_scin_zoom_2_%d.png", resultDir.c_str(), iteration));
