...
bool TimeCalibration::init() {
  ...
  if (!loadOptions()) {
    ERROR("Error while loading options from configuration file. Check user "
          "options!");
    return false;
  }
  ...
  INFO("#############");
  INFO("WE ARE GOING TO CALIBRATE SCINTILLATOR " +
       std::to_string(fStripToCalib) + " FROM LAYER " +
       std::to_string(fLayerToCalib));
  fTimer.startMeasurement();
  fTimeConstantsCalibFileNameTmp = std::to_string(fStripToCalib) + "_" +
                                   std::to_string(fLayerToCalib) + "_" +
                                   fTimeConstantsCalibFileNameTmp;
  ...
  if (!inFile1) {
    ...
    inFile1.close();
    if (Niter == 1) {
      std::fstream inFile2(fTimeConstantsCalibFileName,
                           std::ios::out | std::ios::app);
      inFile2 << "#Calibration started on "
              << JPetCommonTools::currentDateTime() << std::endl;
      inFile2.close();
    }
  }
  if (fIsCorrection) {
    for (int i = 1; i <= kNumberOfThresholds; i++) {
      ...
      CAlCor[i] = CAlTmp[i];
      CBlCor[i] = CBlTmp[i];
      INFO("Iteration: " + std::to_string(Niter) + ", CONSTANTS: " +
           std::to_string(CAtCor[i]) + ", " + std::to_string(CBtCor[i]) + ", " +
           std::to_string(CAlCor[i]) + ", " + std::to_string(CBlCor[i]));
    }
  }
  createHistograms();
  INFO("#############");
...
bool TimeCalibration::loadOptions() {
  ...
  std::vector<std::string> requiredOptions = {
      kTOTCutLowOptName,     kTOTCutHighOptName,   kMainStripOptName,
      kLoadConstantsOptName, kCalibFileTmpOptName, kCalibFileFinalOptName,
      kPMIdRefOptName,       MaxIterNumOptName};

  auto allOptionsExist =
      std::all_of(requiredOptions.begin(), requiredOptions.end(),
                  [&opts](std::string optName) -> bool {
                    auto ok = isOptionSet(opts, optName);
                    if (!ok) {
                      ERROR("No option " + optName +
                            " or bad option format in user param json file.");
                    }
                    return ok;
                  });
  if (allOptionsExist) {
    ...
    fTimeConstantsCalibFileNameTmp =
        getOptionAsString(opts, kCalibFileTmpOptName);
    fTimeConstantsCalibFileName =
        getOptionAsString(opts, kCalibFileFinalOptName);
...
void TimeCalibration::createHistograms() {
  ...
  for (int thr = 1; thr <= kNumberOfThresholds; thr++) {
    ...
    const char *histo_name_l =
        Form("%slayer_%d_slot_%d_thr_%d", "timeDiffAB_leading_", fLayerToCalib,
             fStripToCalib, thr);
    getStatistics().createHistogram(
        new TH1F(histo_name_l, histo_name_l, NbinABl, tABlmin, tABlmax));
    const char *histo_name_Ref_l =
        Form("%slayer_%d_slot_%d_thr_%d", "timeDiffRef_leading_", fLayerToCalib,
             fStripToCalib, thr);
    getStatistics().createHistogram(new TH1F(histo_name_Ref_l, histo_name_Ref_l,
                                             NbinReffl, tRefflMin, tRefflMax));
    const char *histo_name_t =
        Form("%slayer_%d_slot_%d_thr_%d", "timeDiffAB_trailing_", fLayerToCalib,
             fStripToCalib, thr);
    getStatistics().createHistogram(
        new TH1F(histo_name_t, histo_name_t, NbinABt, tABtmin, tABtmax));
    const char *histo_name_Ref_t =
        Form("%slayer_%d_slot_%d_thr_%d", "timeDiffRef_trailing_",
             fLayerToCalib, fStripToCalib, thr);
    getStatistics().createHistogram(new TH1F(histo_name_Ref_t, histo_name_Ref_t,
                                             NbinRefft, tRefftMin, tRefftMax));
  }
}
...
bool TimeCalibration::exec() {
  ...
  if (auto timeWindow = dynamic_cast<const JPetTimeWindow *const>(fEvent)) {
    ...
    for (auto i = 0u; i < n; ++i) {
      const JPetHit &hit =
          dynamic_cast<const JPetHit &>(timeWindow->operator[](i));
      int PMid =
          hit.getSignalB().getRecoSignal().getRawSignal().getPM().getID();
      if (PMid == kPMIdRef) {
        auto lead_times_B = hit.getSignalB()
                                .getRecoSignal()
                                .getRawSignal()
...
bool TimeCalibration::terminate() {
  fitAndSaveParametersToFile(fTimeConstantsCalibFileName,
                             fTimeConstantsCalibFileNameTmp);
...
void TimeCalibration::fillHistosForHit(const JPetHit &hit,
                                       const std::vector<double> &refTimesL,
  ...
  if (tTOT >= TOTcut[0] && tTOT <= TOTcut[1]) {
    for (auto &thr_time_pair : lead_times_A) {
      ...
      if (lead_times_B.count(thr) > 0) {
        double timeDiffAB_l = (lead_times_B[thr] / 1000. + CBlCor[thr]) -
                              (lead_times_A[thr] / 1000. + CAlCor[thr]);
        const char *histo_name_l = formatUniqueSlotDescription(
            hit.getBarrelSlot(), thr, "timeDiffAB_leading_");
        ...
        for (unsigned int i = 0; i < refTimesL.size(); i++) {
          double timeDiffHit_L = (lead_times_A[thr] / 1000. + CAlCor[thr]) +
                                 (lead_times_B[thr] / 1000. + CBlCor[thr]);
          timeDiffHit_L = timeDiffHit_L / 2. - refTimesL[i];
          if (fabs(timeDiffHit_L) < timeDiffLmin) {
            timeDiffLmin = timeDiffHit_L;
          }
        }
        const char *histo_name_Ref_l = formatUniqueSlotDescription(
            hit.getBarrelSlot(), thr, "timeDiffRef_leading_");
    ...
    for (auto &thr_time_pair : trail_times_A) {
      ...
      if (trail_times_B.count(thr) > 0) {
        ...
        double timeDiffAB_t = (trail_times_B[thr] / 1000. + CBtCor[thr]) -
                              (trail_times_A[thr] / 1000. + CAtCor[thr]);
        const char *histo_name_t = formatUniqueSlotDescription(
            hit.getBarrelSlot(), thr, "timeDiffAB_trailing_");
        ...
        for (unsigned int i = 0; i < refTimesT.size(); i++) {
          double timeDiffHit_T = (trail_times_A[thr] / 1000. + CAtCor[thr]) +
                                 (trail_times_B[thr] / 1000. + CBtCor[thr]);
          timeDiffHit_T = timeDiffHit_T / 2. - refTimesT[i];
          if (fabs(timeDiffHit_T) < timeDiffTmin) {
            timeDiffTmin = timeDiffHit_T;
          }
        }
        const char *histo_name_Ref_t = formatUniqueSlotDescription(
            hit.getBarrelSlot(), thr, "timeDiffRef_trailing_");
...
const char *TimeCalibration::formatUniqueSlotDescription(
    const JPetBarrelSlot &slot, int threshold, const char *prefix = "") {
  assert(fMapper);
  int slot_number = fMapper->getSlotNumber(slot);
  int layer_number = fMapper->getLayerNumber(slot.getLayer());
  return Form("%slayer_%d_slot_%d_thr_%d", prefix, layer_number, slot_number,
              threshold);
}
  ...
  if (inFile) {
    while (getline(inFile, line)) {
      for (int i = 1; i <= kNumberOfThresholds; i++) {
        ...
        inFile >> LayerToCalibTmp >> StripToCalibTmp >> SideTmpA >> thrTmp >>
            CAlTmp[i] >> SigCAlTmp[i] >> CAtTmp[i] >> SigCAtTmp[i] >>
            sigma_peak_Ref_lTmp[i] >> sigma_peak_Ref_tTmp[i] >>
            chi2_ndf_Ref_lATmp[i] >> chi2_ndf_Ref_tATmp[i] >> Niter >>
            peak_Ref_l[i] >> peak_err_Ref_l[i] >> peak_Ref_t[i] >>
            peak_err_Ref_t[i];
        inFile >> LayerToCalibTmp >> StripToCalibTmp >> SideTmpB >> thrTmp >>
            CBlTmp[i] >> SigCBlTmp[i] >> CBtTmp[i] >> SigCBtTmp[i] >>
            sigma_peak_lTmp[i] >> sigma_peak_tTmp[i] >> chi2_ndf_Ref_lBTmp[i] >>
            chi2_ndf_Ref_tBTmp[i] >> Niter >> peak_l[i] >> peak_err_l[i] >>
            peak_t[i] >> peak_err_t[i];
      }
    }
    for (int i = 1; i <= kNumberOfThresholds; i++) {
      INFO(std::to_string(LayerToCalibTmp) + " " +
           std::to_string(StripToCalibTmp) + " " + SideTmpA + " " +
           std::to_string(thrTmp) + " " + std::to_string(CAlTmp[i]) + " " +
           std::to_string(SigCAlTmp[i]) + " " + std::to_string(CAtTmp[i]) +
           " " + std::to_string(SigCAtTmp[i]) + " " +
           std::to_string(sigma_peak_Ref_lTmp[i]) + " " +
           std::to_string(sigma_peak_Ref_tTmp[i]) + " " +
           std::to_string(chi2_ndf_Ref_lATmp[i]) + " " +
           std::to_string(chi2_ndf_Ref_tATmp[i]));
      INFO(std::to_string(LayerToCalibTmp) + " " +
           std::to_string(StripToCalibTmp) + " " + SideTmpB + " " +
           std::to_string(thrTmp) + " " + std::to_string(CBlTmp[i]) + " " +
           std::to_string(SigCBlTmp[i]) + " " + std::to_string(CBtTmp[i]) +
           " " + std::to_string(SigCBtTmp[i]) + " " +
           std::to_string(sigma_peak_lTmp[i]) + " " +
           std::to_string(sigma_peak_tTmp[i]) + " " +
           std::to_string(chi2_ndf_Ref_lBTmp[i]) + " " +
           std::to_string(chi2_ndf_Ref_tBTmp[i]));
    }
    Niter = Niter +
            1; // Increment the iteration number if the file was on the disk.
    INFO("ITERATION NUMBER: " + std::to_string(Niter));
    INFO("Temporary file " + filename + " opened and read at " +
         JPetCommonTools::currentDateTime());
  } else {
    inFile.close();
    std::ofstream inFile1(filename, std::ios::app);
    inFile1 << "# Calibration started on " << JPetCommonTools::currentDateTime()
            << std::endl;
    INFO("Temporary file " + filename + " opened at first iteration at " +
         JPetCommonTools::currentDateTime());
    Niter = 1; // If the tmp file was not present set the iteration number to 1,
               // prevents e.g. removing the tmp file
  }
}

void TimeCalibration::fitAndSaveParametersToFile(
    const std::string &filename, const std::string &tmpFilename) {
  int min_ev = 100; // minimal number of events for a distribution to be fitted
  double frac_err = 0.3; // maximal fractional uncertainty of parameters
  ...
  for (int thr = 1; thr <= kNumberOfThresholds; thr++) {
    ...
    const char *histo_name_Ref_t =
        Form("%slayer_%d_slot_%d_thr_%d", "timeDiffRef_trailing_",
             fLayerToCalib, fStripToCalib, thr);
    auto histoToSave_Ref_trailing =
        getStatistics().getHisto1D(histo_name_Ref_t);
    assert(histoToSave_Ref_trailing);

    if (histoToSave_leading->GetEntries() != 0 &&
        histoToSave_trailing->GetEntries() != 0 &&
        histoToSave_Ref_leading->GetEntries() != 0 &&
        histoToSave_Ref_trailing->GetEntries() != 0) {
      INFO("#############");
      INFO("CALIB_INFO: Fitting histogams for layer= " +
           std::to_string(fLayerToCalib) +
           ", slot= " + std::to_string(fStripToCalib) +
           ", threshold= " + std::to_string(thr));
      INFO("#############");
      if (histoToSave_Ref_leading->GetEntries() <= min_ev) {
        results_fit << "#WARNING: Statistics used to determine the leading "
                       "edge calibration constant with respect to the "
                       "refference detector was less than "
                    << min_ev << " events!" << endl;
        WARNING(
            ": Statistics used to determine the leading edge calibration "
            "constant with respect to the refference detector was less than " +
            std::to_string(min_ev) + " events!");
      }
      if (histoToSave_Ref_trailing->GetEntries() <= min_ev) {
        results_fit << "#WARNING: Statistics used to determine the trailing "
                       "edge calibration constant with respect to the "
                       "refference detector was less than "
                    << min_ev << " events!" << endl;
        WARNING(
            ": Statistics used to determine the trailing edge calibration "
            "constant with respect to the refference detector was less than " +
            std::to_string(min_ev) + " events!");
      }
      if (histoToSave_leading->GetEntries() <= min_ev) {
        results_fit << "#WARNING: Statistics used to determine the leading "
                       "edge A-B calibration constant was less than "
                    << min_ev << " events!" << endl;
        WARNING(": Statistics used to determine the leading edge A-B "
                "calibration constant was less than" +
                std::to_string(min_ev) + " events!");
      }
      if (histoToSave_trailing->GetEntries() <= min_ev) {
        results_fit << "#WARNING: Statistics used to determine the trailing "
                       "edge A-B calibration constant was less than "
                    << min_ev << " events!" << endl;
        WARNING(": Statistics used to determine the trailing edge A-B "
                "calibration constant was less than" +
                std::to_string(min_ev) + " events!");
      }
      int highestBin_l = histoToSave_leading->GetBinCenter(
          histoToSave_leading->GetMaximumBin());
      assert(histoToSave_leading->Integral() > 0);
      histoToSave_leading->Fit("gaus", "", "", highestBin_l - 20,
                               highestBin_l + 20);

      int highestBin_t = histoToSave_trailing->GetBinCenter(
          histoToSave_trailing->GetMaximumBin());
      assert(histoToSave_trailing->Integral() > 0);
      histoToSave_trailing->Fit("gaus", "", "", highestBin_t - 20,
                                highestBin_t + 20);
      ...
      int highestBin_Ref_l = histoToSave_Ref_leading->GetBinCenter(
          histoToSave_Ref_leading->GetMaximumBin());
      assert(histoToSave_Ref_leading->Integral() > 0);
      histoToSave_Ref_leading->Fit("gaus", "", "", highestBin_Ref_l - 5,
                                   highestBin_Ref_l + 5); // range for fit
      TF1 *fit_Ref_l = histoToSave_Ref_leading->GetFunction("gaus");
      assert(fit_Ref_l);
      int highestBin_Ref_t = histoToSave_Ref_trailing->GetBinCenter(
          histoToSave_Ref_trailing->GetMaximumBin());
      assert(histoToSave_Ref_trailing->Integral() > 0);
      histoToSave_Ref_trailing->Fit("gaus", "", "", highestBin_Ref_t - 5,
                                    highestBin_Ref_t + 5); // range for fit
      ...
      SigCAl[thr] = sqrt(pow(position_peak_error_Ref_l[thr] / 2., 2) +
                         pow(position_peak_error_l[thr], 2));
      CAt[thr] = -(position_peak_Ref_t[thr]) + position_peak_t[thr] / 2.;
      SigCAt[thr] = sqrt(pow(position_peak_error_Ref_t[thr] / 2., 2) +
                         pow(position_peak_error_t[thr], 2));
      CBl[thr] = -(position_peak_Ref_l[thr] - Cl[fLayerToCalib - 1]) -
                 position_peak_l[thr] / 2.;
      SigCBl[thr] = SigCAl[thr];
      CBt[thr] = -(position_peak_Ref_t[thr] - Cl[fLayerToCalib - 1]) -
                 position_peak_t[thr] / 2.;
      SigCBt[thr] = SigCAt[thr];
    } else {
      ERROR(": ONE OF THE HISTOGRAMS FOR THRESHOLD " + std::to_string(thr) +
            " LAYER " + std::to_string(fLayerToCalib) + " SLOT " +
            std::to_string(fStripToCalib) +
  ...
  if (CheckIfExit(Niter)) {
    for (int thr = 1; thr <= kNumberOfThresholds; thr++) {
      ...
      SigCAl[thr] =
          sqrt(pow(SigCAl[thr], 2) + pow(SigCl[fLayerToCalib - 1], 2) +
               pow(SigCAlTmp[thr], 2));
      SigCAt[thr] =
          sqrt(pow(SigCAt[thr], 2) + pow(SigCl[fLayerToCalib - 1], 2) +
               pow(SigCAtTmp[thr], 2));
      CBl[thr] = CBl[thr] + CBlTmp[thr] - Cl[fLayerToCalib - 1];
      CBt[thr] = CBt[thr] + CBtTmp[thr] - Cl[fLayerToCalib - 1];
      SigCBl[thr] =
          sqrt(pow(SigCBl[thr], 2) + pow(SigCl[fLayerToCalib - 1], 2) +
               pow(SigCBlTmp[thr], 2));
      SigCBt[thr] =
          sqrt(pow(SigCBt[thr], 2) + pow(SigCl[fLayerToCalib - 1], 2) +
               pow(SigCBtTmp[thr], 2));
      //
      results_fit << fLayerToCalib << "\t" << fStripToCalib << "\t"
                  << "A"
                  << "\t" << thr << "\t" << CAl[thr] << "\t" << SigCAl[thr]
                  << "\t" << CAt[thr] << "\t" << SigCAt[thr] << "\t"
                  << sigma_peak_Ref_l[thr] << "\t" << sigma_peak_Ref_t[thr]
                  << "\t" << chi2_ndf_Ref_l[thr] << "\t" << chi2_ndf_Ref_t[thr]
                  << "\t" << std::endl;
      //
      results_fit << fLayerToCalib << "\t" << fStripToCalib << "\t"
                  << "B"
                  << "\t" << thr << "\t" << CBl[thr] << "\t" << SigCBl[thr]
                  << "\t" << CBt[thr] << "\t" << SigCBt[thr] << "\t"
                  << sigma_peak_l[thr] << "\t" << sigma_peak_t[thr] << "\t"
                  << chi2_ndf_l[thr] << "\t" << chi2_ndf_t[thr] << "\t"
                  << std::endl;

      results_fitTmp << fLayerToCalib << "\t" << fStripToCalib << "\t"
                     << "A"
                     << "\t" << thr << "\t" << CAl[thr] << "\t" << SigCAl[thr]
                     << "\t" << CAt[thr] << "\t" << SigCAt[thr] << "\t"
                     << sigma_peak_Ref_l[thr] << "\t" << sigma_peak_Ref_t[thr]
                     << "\t" << chi2_ndf_Ref_l[thr] << "\t"
                     << chi2_ndf_Ref_t[thr] << "\t" << Niter << "\t"
                     << position_peak_Ref_l[thr] << "\t"
                     << position_peak_error_Ref_l[thr] << "\t"
                     << position_peak_Ref_t[thr] << "\t"
                     << position_peak_error_Ref_t[thr] << Niter << std::endl;
      //
      results_fitTmp << fLayerToCalib << "\t" << fStripToCalib << "\t"
                     << "B"
                     << "\t" << thr << "\t" << CBl[thr] << "\t" << SigCBl[thr]
                     ...
                     << sigma_peak_l[thr] << "\t" << sigma_peak_t[thr] << "\t"
                     << chi2_ndf_l[thr] << "\t" << chi2_ndf_t[thr] << "\t"
                     << Niter << "\t" << position_peak_l[thr] << "\t"
                     << position_peak_error_l[thr] << "\t"
                     << position_peak_t[thr] << "\t"
                     << position_peak_error_t[thr] << Niter << std::endl;
    }
  } else {
    for (int thr = 1; thr <= kNumberOfThresholds; thr++) {
      //
      ...
      if (SigCAl[thr] / abs(CAl[thr]) >= frac_err) {
        results_fitTmp << "#WFIT: Large uncertainty on the calibration "
                          "constant (Side A, leading edge)!"
                       << endl;
      }
      assert(abs(CBl[thr]) != 0);
      if (SigCBl[thr] / abs(CBl[thr]) >= frac_err) {
        results_fitTmp << "#WFIT: Large uncertainty on the calibration "
                          "constant (Side B leading edge)!"
                       << endl;
      }
      assert(abs(CAt[thr]) != 0);
      if (SigCAt[thr] / abs(CAt[thr]) >= frac_err) {
        results_fitTmp << "#WFIT: Large uncertainty on the calibration "
                          "constant (Side A, trailing edge)!"
                       << endl;
      }
      assert(abs(CBl[thr]) != 0);
      if (SigCBt[thr] / abs(CBt[thr]) >= frac_err) {
        results_fitTmp << "#WFIT: Large uncertainty on the calibration "
                          "constant (Side B trailing edge)!"
                       << endl;
      }
      //
      results_fitTmp << fLayerToCalib << "\t" << fStripToCalib << "\t"
                     ...
                     << position_peak_error_Ref_l[thr] << "\t"
                     << position_peak_Ref_t[thr] << "\t"
                     << position_peak_error_Ref_t[thr] << std::endl;
      //
      results_fitTmp << fLayerToCalib << "\t" << fStripToCalib << "\t"
                     << "B"
                     << "\t" << thr << "\t" << CBl[thr] << "\t" << SigCBl[thr]
                     ...
                     << position_peak_error_l[thr] << "\t"
                     << position_peak_t[thr] << "\t"
                     << position_peak_error_t[thr] << std::endl;
    }
  }
  results_fit.close();
...
void TimeCalibration::writeHeader(const std::string &filename) {
  ...
  output << "# For side A we apply only the correction from refference "
            "detector, for side B the correction is equal to the sum of the A-B"
         << std::endl;
  output << "# correction and offset with respect to the refference detector. "
            "For side A we report the sigmas and chi2/ndf for fit to the time "
            "difference spectra with refference detector"
         << std::endl;
  output << "# while the same quality variables for fits to the A-B time "
            "difference are given for B side section"
         << std::endl;
  output << "# Description of the parameters: layer(1-3) | slot(1-48/96) | "
            "side(A-B) | threshold(1-4) | offset_value_leading | "
            "offset_uncertainty_leading | offset_value_trailing | "
            "offset_uncertainty_trailing | sigma_offset_leading | "
            "sigma_offset_trailing | (chi2/ndf)_leading | (chi2/ndf)_trailing|"
         << std::endl;
  output << "# Calibration started on " << JPetCommonTools::currentDateTime()
         << std::endl;
}
