...
protected:
  ...
  const char *formatUniqueSlotDescription(const JPetBarrelSlot &slot,
                                          int threshold, const char *prefix);
  void fillHistosForHit(const JPetHit &hit,
                        const std::vector<double> &refTimesL,
                        const std::vector<double> &refTimesT);
  void fitAndSaveParametersToFile(const std::string &filename,
                                  const std::string &filenameTmp);
  ...
  const std::string kLoadConstantsOptName =
      "TimeCalibration_LoadConstants_bool";
  const std::string kCalibFileTmpOptName =
      "TimeCalibration_OutputFileTmp_std::string";
  const std::string kCalibFileFinalOptName =
      "TimeCalibration_OutputFileFinal_std::string";
  const std::string MaxIterNumOptName = "TimeCalibration_NiterMax_int";
  int kPMIdRef = 385;
  std::array<float, 2> TOTcut{
      {-300000000.,
       300000000.}}; // TOT cuts for slot hits (sum of TOTs from both sides)
  int fLayerToCalib = -1;    // Layer of calibrated slot
  int fStripToCalib = -1;    // Slot to be calibrated
  bool fIsCorrection = true; // Flag for choosing the correction of times at the
                             // level of calibration module (use only if the
                             // calibration loader is not used)
