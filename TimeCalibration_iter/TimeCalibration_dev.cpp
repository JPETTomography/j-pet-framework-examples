...
//------ Packed strip and layer number from config (json) file
	if (opts.count(fConstantsLoadingFlag)) {
	  flag_corr = std::atoi(opts.at(fConstantsLoadingFlag).c_str());
	}
//------ Max number of iterations from config (json) file
	if (opts.count(fMaxIterationNumber)) {
	 NiterMax = std::atoi(opts.at(fMaxIterationNumber).c_str());
	}
//------ Temporary file name(the same as the name of file for CalibLoader 
	if (opts.count(fTmpOutFile)) {
	  OutputFileTmp =opts.at(fTmpOutFile) ;
	}
	//
	INFO("#############"); 
...
//offset = -C2 (ref. det) -C1/2 (AB calib)        
	       ...
	       //
	       if(CheckIfExitIter(CAl,SigCAl,CBl,SigCBl,CAt,SigCAt,CBt,SigCBt,Niter)){
