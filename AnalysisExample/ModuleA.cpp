...
void ModuleA::createInputObjects(const char* inputFilename) {
  ...
  if (fHLDReader->openFile(fInFileName.c_str()) == 0) {
    std::cerr << fName << ": Unable to open the input file. Exiting."
              << std::endl;
    exit(-1);
  }
  fHLDReader->readData();
  fEventNb = fHLDReader->getEntries();
...
void ModuleA::exec() {
  // Get HLD Event
  fHLDReader->getEntry(fEvent);
  Event* evt;
  evt = (Event*) (&(fHLDReader->getData()));
