...
void ModuleC1::exec() {
  ...
  JPetTSlot & tslot = (JPetTSlot&) fReader->getData();
  ...
  for (int i = 0; i < getParamBank().getPMsSize(); i++) {
    ...
    if (sig.getNPoints(JPetSigCh::Leading) == 0
        && sig.getNPoints(JPetSigCh::Trailing) == 0) { //skip empty signals
      continue;
    }
    ...
    getHisto1D("No. leading points")->Fill(sig.getNPoints(JPetSigCh::Leading));
    getHisto1D("No. trailing points")->Fill(
        sig.getNPoints(JPetSigCh::Trailing));
    getHisto2D("leading vs trailing")->Fill(
        sig.getNPoints(JPetSigCh::Leading),
        sig.getNPoints(JPetSigCh::Trailing));

    getHisto1D("PM multiplicity")->Fill(sig.getPM().getID());

    // consider events with at least 3 fired thresholds
    if (sig.getNPoints(JPetSigCh::Leading) >= 2
        && sig.getNPoints(JPetSigCh::Trailing) >= 2) {
