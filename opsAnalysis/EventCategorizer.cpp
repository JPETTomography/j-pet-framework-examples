    ...
    for (uint i = 0; i < timeWindow->getNumberOfEvents(); i++)
    {
      ...
      if (event.getHits().size() == 1)
      {
        ...
        if (!fIsMC)
        {
          ...
          if (tot_hits > 5.5)
          {
            pmtEmmTime = hit->getTime() / 1000 - hit->getPos().Mag() / kLightVelocity_cm_ns;
...
void EventCategorizer::reconHitsCalculation(vector<const JPetPhysRecoHit*> hits, EvtType& event_type, const JPetTimeWindowMC& time_window_mc)
{
  ...
  time.push_back(momenta[0].Mag() / kLightVelocity_cm_ns);
  time.push_back(momenta[1].Mag() / kLightVelocity_cm_ns);
  time.push_back(momenta[2].Mag() / kLightVelocity_cm_ns);
  ...
  energies[0] =
      -2 * mass_e * (-cos(theta[2]) + cos(theta[0]) * cos(theta[1])) / ((-1 + cos(theta[0])) * (1 + cos(theta[0]) - cos(theta[2]) - cos(theta[1])));
  energies[1] =
      -2 * mass_e * (cos(theta[0]) * cos(theta[2]) - cos(theta[1])) / ((-1 + cos(theta[0])) * (1 + cos(theta[0]) - cos(theta[2]) - cos(theta[1])));
  ...
  getStatistics().fillHistogram(("angle_dLOR_beforeTri_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                calculatedLOR(hits, event_type));
  getStatistics().fillHistogram(("E1E2_beforeTri_" + EventTypeNames.at(event_type)).c_str(), energies.at(0), energies.at(1));
  getStatistics().fillHistogram(("theta12_beforeTri_" + EventTypeNames.at(event_type)).c_str(), theta_rndm.at(0) * TMath::RadToDeg(),
                                theta_rndm.at(1) * TMath::RadToDeg());
  getStatistics().fillHistogram(("sumDiff_angles_beforeTri_" + EventTypeNames.at(event_type)).c_str(),
                                (theta.at(0) + theta.at(1)) * TMath::RadToDeg(), (theta.at(1) - theta.at(0)) * TMath::RadToDeg());
  getStatistics().fillHistogram(("sumDiff_azmTheta_beforeTri_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                (thetas[1] - thetas[0]));

  if (error == 0)
  {
    getStatistics().fillHistogram("HitMult_afterTri", event_type);
    getStatistics().fillHistogram(("sumDiff_angles_afterTri_" + EventTypeNames.at(event_type)).c_str(),
                                  (theta.at(0) + theta.at(1)) * TMath::RadToDeg(), (theta.at(1) - theta.at(0)) * TMath::RadToDeg());
    getStatistics().fillHistogram(("sumDiff_azmTheta_afterTri_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                  (thetas[1] - thetas[0]));
    getStatistics().fillHistogram(("angle_dLOR_afterTri_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                  calculatedLOR(hits, event_type));
    getStatistics().fillHistogram(("theta12_afterTri_" + EventTypeNames.at(event_type)).c_str(), theta_rndm.at(0) * TMath::RadToDeg(),
                                  theta_rndm.at(1) * TMath::RadToDeg());
    ...
    if (energies[0] > 0 && energies[1] > 0 && energies[2] > 0)
    {
      ...
      getStatistics().fillHistogram(("theta12_afterEng_" + EventTypeNames.at(event_type)).c_str(), theta_rndm.at(0) * TMath::RadToDeg(),
                                    theta_rndm.at(1) * TMath::RadToDeg());
      getStatistics().fillHistogram(("scattTest_afterEng_" + EventTypeNames.at(event_type)).c_str(), scatterTest(hits, event_type));
      getStatistics().fillHistogram(("sumDiff_angles_afterEng_" + EventTypeNames.at(event_type)).c_str(),
                                    (theta.at(0) + theta.at(1)) * TMath::RadToDeg(), (theta.at(1) - theta.at(0)) * TMath::RadToDeg());
      getStatistics().fillHistogram(("sumDiff_azmTheta_afterEng_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                    (thetas[1] - thetas[0]));
      getStatistics().fillHistogram(("angle_dLOR_afterEng_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                    calculatedLOR(hits, event_type));
      ...
      if (hit1->getEnergy() / 1000000 < 5 && hit2->getEnergy() / 1000000 < 5 && hit3->getEnergy() / 1000000 < 5)
      {
        ...
        getStatistics().fillHistogram(("angle_dLOR_afterEngDep_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                      calculatedLOR(hits, event_type));
        getStatistics().fillHistogram(("E1E2_afterEngDep_" + EventTypeNames.at(event_type)).c_str(), energies.at(0), energies.at(1));
        getStatistics().fillHistogram(("theta12_afterEngDep_" + EventTypeNames.at(event_type)).c_str(), theta_rndm.at(0) * TMath::RadToDeg(),
                                      theta_rndm.at(1) * TMath::RadToDeg());
        getStatistics().fillHistogram(("sumDiff_angles_afterEngDep_" + EventTypeNames.at(event_type)).c_str(),
                                      (theta.at(0) + theta.at(1)) * TMath::RadToDeg(), (theta.at(1) - theta.at(0)) * TMath::RadToDeg());
        getStatistics().fillHistogram(("sumDiff_azmTheta_afterEngDep_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                      (thetas[1] - thetas[0]));
        ...
        if (scatterTest(hits, event_type) > 10)
        {
          ...
          getStatistics().fillHistogram(("sumDiff_angles_afterSTest_" + EventTypeNames.at(event_type)).c_str(),
                                        (theta.at(0) + theta.at(1)) * TMath::RadToDeg(), (theta.at(1) - theta.at(0)) * TMath::RadToDeg());
          getStatistics().fillHistogram(("sumDiff_azmTheta_afterSTest_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                        (thetas[1] - thetas[0]));
          getStatistics().fillHistogram(("angle_dLOR_afterSTest_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                        calculatedLOR(hits, event_type));
          getStatistics().fillHistogram(("theta12_afterSTest_" + EventTypeNames.at(event_type)).c_str(), theta_rndm.at(0) * TMath::RadToDeg(),
                                        theta_rndm.at(1) * TMath::RadToDeg());
          ...
          if ((thetas[0] + thetas[1]) > 210)
          {
            ...
            getStatistics().fillHistogram(("sumDiff_angles_after2DAngle_" + EventTypeNames.at(event_type)).c_str(),
                                          (theta.at(0) + theta.at(1)) * TMath::RadToDeg(), (theta.at(1) - theta.at(0)) * TMath::RadToDeg());
            getStatistics().fillHistogram(("sumDiff_azmTheta_after2DAngle_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                          (thetas[1] - thetas[0]));
            getStatistics().fillHistogram(("angle_dLOR_after2DAngle_" + EventTypeNames.at(event_type)).c_str(), (thetas[0] + thetas[1]),
                                          calculatedLOR(hits, event_type));
            getStatistics().fillHistogram(("theta12_after2DAngle_" + EventTypeNames.at(event_type)).c_str(), theta_rndm.at(0) * TMath::RadToDeg(),
                                          theta_rndm.at(1) * TMath::RadToDeg());
            ...
            getStatistics()
                .getHisto1D(("annh_radius3D_cyd_jacobian_" + EventTypeNames.at(event_type)).c_str())
                ->Fill(decayPoint.Mag(), 1. / decayPoint.Mag());

            getStatistics().getHisto1D(("annh_radius_sph_jacobian_" + EventTypeNames.at(event_type)).c_str())->Fill(sph_jacobian, 1. / sph_jacobian);
            getStatistics()
                .getHisto1D(("annh_radius2D_sph_jacobian_" + EventTypeNames.at(event_type)).c_str())
                ->Fill(pow(transRad, 2), 1. / pow(transRad, 2));
            getStatistics()
                .getHisto1D(("annh_radius3D_sph_jacobian_" + EventTypeNames.at(event_type)).c_str())
                ->Fill(pow(decayPoint.Mag(), 2), 1. / pow(decayPoint.Mag(), 2));
            ...
            for (int n = 0; n < z_position.size(); n++)
            {
              if (sol[1].Z() < z_position.at(n) && sol[1].Z() > -z_position.at(n))
              {
                ...
                getStatistics().fillHistogram(Form(("AnnhPointXY_Zto%d_" + EventTypeNames.at(event_type)).c_str(), z_position.at(n)), sol[1].X(),
                                              sol[1].Y());
              }
            }
...
std::vector<const JPetPhysRecoHit*> EventCategorizer::scatTest(std::vector<const JPetPhysRecoHit*> hits)
{
  ...
  for (int i = 0; i < hits.size(); ++i)
  {
    for (int j = i + 1; j < hits.size(); ++j)
    {
      ...
      float distance = sqrt(pow(hits.at(j)->getPosX() - hits.at(i)->getPosX(), 2) + pow(hits.at(j)->getPosY() - hits.at(i)->getPosY(), 2) +
                            pow(hits.at(j)->getPosZ() - hits.at(i)->getPosZ(), 2));
...
double EventCategorizer::calculatedLOR(vector<const JPetPhysRecoHit*> hits, EvtType event_type)
{
  ...
  for (int i = 0; i < hits.size(); ++i)
  {
    for (int j = i + 1; j < hits.size(); ++j)
    {
      TVector3 vtx2g = EventCategorizerTools::calculateAnnihilationPoint(hits.at(i), hits.at(j));
...
std::vector<double> EventCategorizer::distance_hits(vector<const JPetPhysRecoHit*> hits)
{
  ...
  for (int i = 0; i < hits.size(); ++i)
  {
    for (int j = i + 1; j < hits.size(); ++j)
    {
      ...
      distance.push_back(sqrt(pow(hits.at(j)->getPosX() - hits.at(i)->getPosX(), 2) + pow(hits.at(j)->getPosY() - hits.at(i)->getPosY(), 2) +
                              pow(hits.at(j)->getPosZ() - hits.at(i)->getPosZ(), 2)));
    }
  }
...
void EventCategorizer::initialiseHistograms()
{
  ...
  for (auto& i : EventTypeNames)
  {
    ...
    getStatistics().createHistogramWithAxes(new TH1D(("scattTest_beforeTri_" + i.second).c_str(), "Scatter Test", 120, -10.5, 119.5), "#delta d [cm]",
                                            "Counts");
    getStatistics().createHistogramWithAxes(new TH1D(("scattTest_afterTri_" + i.second).c_str(), "Scatter Test", 120, -10.5, 119.5), "#delta d [cm]",
                                            "Counts");
    getStatistics().createHistogramWithAxes(new TH1D(("scattTest_afterEng_" + i.second).c_str(), "Scatter Test", 120, -10.5, 119.5), "#delta d [cm]",
                                            "Counts");
    getStatistics().createHistogramWithAxes(new TH1D(("scattTest_afterEngDep_" + i.second).c_str(), "Scatter Test", 120, -10.5, 119.5),
                                            "#delta d [cm]", "Counts");
    getStatistics().createHistogramWithAxes(new TH1D(("scattTest_after2DAngle_" + i.second).c_str(), "Scatter Test", 120, -10.5, 119.5),
                                            "#delta d [cm]", "Counts");

    getStatistics().createHistogramWithAxes(new TH1D(("AnnhPointZComp_" + i.second).c_str(), " Annihilation Point (Z) ", 60, -60, 60),
                                            " AnnihilationPoint_Z [cm]", "Counts");

    getStatistics().createHistogramWithAxes(new TH2D(("AnnhPointYZComp_" + i.second).c_str(), "Annihilation Point YZ ", 60, -60, 60, 60, -60, 60),
                                            " AnnihilationPoint_Z [cm]", "AnnihilationPoint_Y [cm]");
    getStatistics().createHistogramWithAxes(new TH2D(("AnnhPointXYComp_" + i.second).c_str(), " Annihilation Point XY ", 60, -60, 60, 60, -60, 60),
                                            " AnnihilationPoint_X [cm]", "AnnihilationPoint_Y [cm]");

    getStatistics().createHistogramWithAxes(new TH2D(("hit_xy_" + i.second).c_str(), " Hit position", 120, -60, 60, 120, -60, 60), "  X [cm]",
                                            "Y [cm]");
    getStatistics().createHistogramWithAxes(new TH2D(("hit_xz_" + i.second).c_str(), " Hit position ", 120, -60, 60, 120, -60, 60), " X [cm]",
                                            "Z [cm]");
    ...
    getStatistics().createHistogramWithAxes(new TH2D(("mchit_xy_" + i.second).c_str(), " Hit position", 120, -60, 60, 120, -60, 60), "  X [cm]",
                                            "Y [cm]");
    getStatistics().createHistogramWithAxes(new TH2D(("mchit_xz_" + i.second).c_str(), " Hit position ", 120, -60, 60, 120, -60, 60), " X [cm]",
                                            "Z [cm]");

    getStatistics().createHistogramWithAxes(new TH2D(("theta12_beforeTri_" + i.second).c_str(), "Angle12_vs_Angle23", 200, 0., 200., 200, 0., 200.),
                                            "#theta_{12} [deg]", "#theta_{23} [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("theta12_afterSTest_" + i.second).c_str(), "Angle12_vs_Angle23 ", 200, 0., 200., 200, 0., 200.),
                                            "#theta_{12} [deg]", "#theta_{23} [deg]");
    getStatistics().createHistogramWithAxes(
        new TH2D(("theta12_afterEngDep_" + i.second).c_str(), "Angle12_vs_Angle23 ", 200, 0., 200., 200, 0., 200.), "#theta_{12} [deg]",
        "#theta_{23} [deg]");
    getStatistics().createHistogramWithAxes(
        new TH2D(("theta12_after2DAngle_" + i.second).c_str(), "Angle12_vs_Angle23 ", 200, 0., 200., 200, 0., 200.), "#theta_{12} [deg]",
        "#theta_{23} [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("theta12_afterTri_" + i.second).c_str(), "Angle12_vs_Angle23", 200, 0., 200., 200, 0., 200.),
                                            "#theta_{12} [deg]", "#theta_{23} [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("theta12_afterEng_" + i.second).c_str(), "Angle12_vs_Angle23", 200, 0., 200., 200, 0., 200.),
                                            "#theta_{12} [deg]", "#theta_{23} [deg]");

    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_angles_beforeTri_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "#theta_{1} + #theta_{2} [deg]", "#theta_{2} - #theta_{1} [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_angles_afterEngDep_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "#theta_{1} + #theta_{2} [deg]", "#theta_{2} - #theta_{1} [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_angles_afterSTest_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "#theta_{1} + #theta_{2} [deg]", "#theta_{2} - #theta_{1} [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_angles_after2DAngle_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "#theta_{1} + #theta_{2} [deg]", "#theta_{2} - #theta_{1} [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_angles_afterTri_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "#theta_{1} + #theta_{2} [deg]", "#theta_{2} - #theta_{1} [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_angles_afterEng_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "#theta_{1} + #theta_{2} [deg]", "#theta_{2} - #theta_{1} [deg]");

    getStatistics().createHistogramWithAxes(new TH2D(("angle_dLOR_beforeTri_" + i.second).c_str(), " ", 300, -0.5, 299.5, 120, -0.5, 59.5),
                                            "#theta_{1} + #theta_{2} [deg]", "min d_{LOR} [cm]");
    getStatistics().createHistogramWithAxes(new TH2D(("angle_dLOR_afterEng_" + i.second).c_str(), " ", 300, -0.5, 299.5, 120, -0.5, 59.5),
                                            "#theta_{1} + #theta_{2} [deg]", "min d_{LOR} [cm]");
    getStatistics().createHistogramWithAxes(new TH2D(("angle_dLOR_afterEngDep_" + i.second).c_str(), " ", 300, -0.5, 299.5, 120, -0.5, 59.5),
                                            "#theta_{1} + #theta_{2} [deg]", "min d_{LOR} [cm]");
    getStatistics().createHistogramWithAxes(new TH2D(("angle_dLOR_afterSTest_" + i.second).c_str(), " ", 300, -0.5, 299.5, 120, -0.5, 59.5),
                                            "#theta_{1} + #theta_{2} [deg]", "min d_{LOR} [cm]");
    getStatistics().createHistogramWithAxes(new TH2D(("angle_dLOR_after2DAngle_" + i.second).c_str(), " ", 300, -0.5, 299.5, 120, -0.5, 59.5),
                                            "#theta_{1} + #theta_{2} [deg]", "min d_{LOR} [cm]");
    getStatistics().createHistogramWithAxes(new TH2D(("angle_dLOR_afterTri_" + i.second).c_str(), " ", 300, -0.5, 299.5, 120, -0.5, 59.5),
                                            "#theta_{1} + #theta_{2} [deg]", "min d_{LOR} [cm]");

    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_azmTheta_afterEngDep_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "Sum of 2 smallest angles [deg]", "Diff between 2 smallest angles [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_azmTheta_beforeTri_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "Sum of 2 smallest angles [deg]", "Diff between 2 smallest angles [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_azmTheta_afterSTest_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "Sum of 2 smallest angles [deg]", "Diff between 2 smallest angles [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_azmTheta_after2DAngle_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "Sum of 2 smallest angles [deg]", "Diff between 2 smallest angles [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_azmTheta_afterTri_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "Sum of 2 smallest angles [deg]", "Diff between 2 smallest angles [deg]");
    getStatistics().createHistogramWithAxes(new TH2D(("sumDiff_azmTheta_afterEng_" + i.second).c_str(), " ", 150, -0.5, 299.5, 150, -0.5, 299.5),
                                            "Sum of 2 smallest angles [deg]", "Diff between 2 smallest angles [deg]");

    getStatistics().createHistogramWithAxes(new TH2D(("E1E2_beforeTri_" + i.second).c_str(), "E1 vs E2 ", 301, -0.5, 599.5, 301, -0.5, 599.5),
                                            "E_{1} [keV]", "E_{2} [keV]");
    getStatistics().createHistogramWithAxes(new TH2D(("E1E2_afterEngDep_" + i.second).c_str(), "E1 vs E2 ", 301, -0.5, 599.5, 301, -0.5, 599.5),
                                            "E_{1} [keV]", "E_{2} [keV]");
    getStatistics().createHistogramWithAxes(new TH2D(("E1E2_afterSTest_" + i.second).c_str(), "E1 vs E2 ", 301, -0.5, 599.5, 301, -0.5, 599.5),
                                            "E_{1} [keV]", "E_{2} [keV]");
    getStatistics().createHistogramWithAxes(new TH2D(("E1E2_after2DAngle_" + i.second).c_str(), "E1 vs E2 ", 301, -0.5, 599.5, 301, -0.5, 599.5),
                                            "E_{1} [keV]", "E_{2} [keV]");
    getStatistics().createHistogramWithAxes(new TH2D(("E1E2_afterTri_" + i.second).c_str(), "E1 vs E2 ", 301, -0.5, 599.5, 301, -0.5, 599.5),
                                            "E_{1} [keV]", "E_{2} [keV]");
    getStatistics().createHistogramWithAxes(new TH2D(("E1E2_afterEng_" + i.second).c_str(), "E1 vs E2 ", 301, -0.5, 599.5, 301, -0.5, 599.5),
                                            "E_{1} [keV]", "E_{2} [keV]");
    ...
    getStatistics().createHistogramWithAxes(new TH1F(("annh_radius_trans2D_" + i.second).c_str(), "Transverse radius ", 120, -60, 60), " radius [cm]",
                                            "Counts");
    getStatistics().createHistogramWithAxes(new TH1F(("annh_radius_3D_" + i.second).c_str(), "Transverse radius ", 120, -60, 60), " radius [cm]",
                                            "Counts");

    getStatistics().createHistogram(new TH1F(("annh_radius_cyd_jacobian_" + i.second).c_str(),
                                             "Transverse radius (weighted);"
                                             " radius [cm]; Counts",
                                             120, -60, 60));
    getStatistics().createHistogram(new TH1F(("annh_radius2D_cyd_jacobian_" + i.second).c_str(),
                                             "Transverse radius (weighted);"
                                             " radius [cm]; Counts",
                                             120, -60, 60));
    getStatistics().createHistogram(new TH1F(("annh_radius3D_cyd_jacobian_" + i.second).c_str(),
                                             "Transverse radius (weighted);"
                                             " radius [cm]; Counts",
                                             120, -60, 60));

    getStatistics().createHistogram(new TH1F(("annh_radius_sph_jacobian_" + i.second).c_str(),
                                             "Transverse radius (weighted);"
                                             " radius [cm]; Counts",
                                             120, -60, 60));
    getStatistics().createHistogram(new TH1F(("annh_radius2D_sph_jacobian_" + i.second).c_str(),
                                             "Transverse radius (weighted);"
                                             " radius [cm]; Counts",
                                             120, -60, 60));
    getStatistics().createHistogram(new TH1F(("annh_radius3D_sph_jacobian_" + i.second).c_str(),
                                             "Transverse radius (weighted);"
                                             " radius [cm]; Counts",
                                             120, -60, 60));
    ...
    {
      getStatistics().createHistogramWithAxes(
          new TH2D(Form(("AnnhPointXY_Zto%d_" + i.second).c_str(), z_position.at(n)), "Annihilation Point XY ", 60, -60, 60, 60, -60, 60),
          " AnnihilationPoint_X [cm]", "AnnihilationPoint_Y [cm]");
    }
  ...
  getStatistics().createHistogramWithAxes(new TH2D("HitMult_ScinID", "Number of Hits v/s scinID", 200, -0.5, 199.5, 11, -0.5, 10.5), "Scin_ID",
                                          "Number of Hits");
