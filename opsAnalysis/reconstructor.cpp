...
Double_t Reconstructor::getGammaTime(int i) const { return gammas_time[i] + gammas_time_shift[i]; }
 ...
 */
void Reconstructor::setBarrelLength(double length) { fStripLength = length; }
...
int Reconstructor::reconstruct()
{
  ...
  { // k - rows
    ...
    D[k] = pow(gammas2D[i].X(), 2.) - pow(gammas2D[j].X(), 2.) + pow(gammas2D[i].Y(), 2.) - pow(gammas2D[j].Y(), 2.) -
           cvel * cvel * pow(getGammaTime(i), 2.) + cvel * cvel * pow(getGammaTime(j), 2.);
  }
  ...
  */
  ...
  if (errFlag == 0)
  {
    ...
    else if (sol_hit[1].Perp() > getGammaHit(1).Perp() || fabs(sol_hit[1].Z()) > fStripLength / 2.)
    {
      errFlag = 3;
    }
    else if (TMath::IsNaN(sol_time[1]) || TMath::IsNaN(sol_hit[1].X()) || TMath::IsNaN(sol_hit[1].Y()) || TMath::IsNaN(sol_hit[1].Z()))
    {
      errFlag = 4;
    }
  }
...
double Reconstructor::getPSlifetime() { return sol_time[1] - getPScreationTime(); }
...
{
  TVector3 vecSurface;
  vecSurface.SetX( (Hit2.getPosY() - Hit1.getPosY() )*(Hit3.getPosZ() - Hit1.getPosZ() ) - (Hit2.getPosZ() - Hit1.getPosZ() )*(Hit3.getPosY() -
Hit1.getPosY() ) ); vecSurface(1) = (Hit2.getPosZ() - Hit1.getPosZ() )*(Hit3.getPosX() - Hit1.getPosX() ) - (Hit2.getPosX() - Hit1.getPosX()
)*(Hit3.getPosZ() - Hit1.getPosZ() ); vecSurface(2) = (Hit2.getPosX() - Hit1.getPosX() )*(Hit3.getPosY() - Hit1.getPosY() ) - (Hit2.getPosY() -
Hit1.getPosY() )*(Hit3.getPosX() - Hit1.getPosX() );
  ...
  TVector3 RotationX( cos(Theta)+vecPerpendicular(0)*vecPerpendicular(0)*(1-cos(Theta)), vecPerpendicular(0)*vecPerpendicular(1)*(1-cos(Theta)),
vecPerpendicular(1)*sin(Theta) ); TVector3 RotationY( vecPerpendicular(0)*vecPerpendicular(1)*(1-cos(Theta)),
cos(Theta)+vecPerpendicular(1)*vecPerpendicular(1)*(1-cos(Theta)), -vecPerpendicular(0)*sin(Theta) ); TVector3 RotationZ(
-vecPerpendicular(1)*sin(Theta), vecPerpendicular(0)*sin(Theta), cos(Theta) );
  ...
  TVector3 RotationXr( cos(-Theta)+vecPerpendicular(0)*vecPerpendicular(0)*(1-cos(-Theta)), vecPerpendicular(0)*vecPerpendicular(1)*(1-cos(-Theta)),
vecPerpendicular(1)*sin(-Theta) ); TVector3 RotationYr( vecPerpendicular(0)*vecPerpendicular(1)*(1-cos(-Theta)),
cos(-Theta)+vecPerpendicular(1)*vecPerpendicular(1)*(1-cos(-Theta)), -vecPerpendicular(0)*sin(-Theta) ); TVector3 RotationZr(
-vecPerpendicular(1)*sin(-Theta), vecPerpendicular(0)*sin(-Theta), cos(-Theta) );
 ...
 while( test )
 {
   ...
   for( unsigned i=0; i<2; i++ )
   {
     for( unsigned j=0; j<2; j++ )
     {
       ...
       {
         Distance = sqrt( pow(Points[i][0] - Points[j+2][0], 2) + pow(Points[i][1] - Points[j+2][1],2 ) ) + sqrt( pow(Points[i][0] - Points[k+4][0],
2) + pow(Points[i][1] - Points[k+4][1],2 ) ) + sqrt( pow(Points[k+4][0] - Points[j+2][0], 2) + pow(Points[k+4][1] - Points[j+2][1],2 ) ); if( Distance
< MinDistance )
         {
...
double Reconstructor::FindMinFromDerrivative(  vector<double> Arg,  vector<double> Val )
{
  ...
  double a = ( Val[StopInd+1] - Val[StopInd] - (Val[StopInd] - Val[StopInd-1]) )/( (Arg[StopInd+1] + Arg[StopInd])/2 - (Arg[StopInd] +
Arg[StopInd-1])/2 ); double b = Val[StopInd+1] - Val[StopInd] - a*(Arg[StopInd+1] + Arg[StopInd])/2; if( a ) return -b/a; else
    {
...
vector< vector<double> > Reconstructor::FindIntersectionPointsOfCircles( TVector3 Hit1Pos, TVector3 Hit2Pos, TVector3 Hit3Pos, double R1, double R2,
double R3, double R13, double R21, double R32 )
{
  ...
  Points[0][0] = (Hit1Pos(0) + Hit2Pos(0) )/2 + (pow(R1,2) - pow(R2,2) )*( Hit2Pos(0) - Hit1Pos(0) )/2/pow( R21,2 ) + 0.5*( Hit2Pos(1) - Hit1Pos(1)
)*sqrt( 2 * (pow(R1,2) + pow(R2,2) )/pow( R21,2 ) - pow( pow(R1,2) - pow(R2,2), 2 )/pow( R21, 4 ) - 1 ); Points[0][1] = (Hit1Pos(1) + Hit2Pos(1) )/2 +
(pow(R1,2) - pow(R2,2) )*( Hit2Pos(1) - Hit1Pos(1) )/2/pow( R21,2 ) + 0.5*( Hit1Pos(0) - Hit2Pos(0) )*sqrt( 2 * (pow(R1,2) + pow(R2,2) )/pow( R21,2 )
- pow( pow(R1,2) - pow(R2,2), 2 )/pow( R21, 4 ) - 1 ); Points[1][0] = (Hit1Pos(0) + Hit2Pos(0) )/2 + (pow(R1,2) - pow(R2,2) )*( Hit2Pos(0) -
Hit1Pos(0) )/2/pow( R21,2 ) - 0.5*( Hit2Pos(1) - Hit1Pos(1) )*sqrt( 2 * (pow(R1,2) + pow(R2,2) )/pow( R21,2 ) - pow( pow(R1,2) - pow(R2,2), 2 )/pow(
R21, 4 ) - 1 ); Points[1][1] = (Hit1Pos(1) + Hit2Pos(1) )/2 + (pow(R1,2) - pow(R2,2) )*( Hit2Pos(1) - Hit1Pos(1) )/2/pow( R21,2 ) - 0.5*( Hit1Pos(0) -
Hit2Pos(0) )*sqrt( 2 * (pow(R1,2) + pow(R2,2) )/pow( R21,2 ) - pow( pow(R1,2) - pow(R2,2), 2 )/pow( R21, 4 ) - 1 );

  Points[2][0] = (Hit2Pos(0) + Hit3Pos(0) )/2 + (pow(R2,2) - pow(R3,2) )*( Hit3Pos(0) - Hit2Pos(0) )/2/pow( R32,2 ) + 0.5*( Hit3Pos(1) - Hit2Pos(1)
)*sqrt( 2 * (pow(R2,2) + pow(R3,2) )/pow( R32,2 ) - pow( pow(R2,2) - pow(R3,2), 2 )/pow( R32, 4 ) - 1 ); Points[2][1] = (Hit2Pos(1) + Hit3Pos(1) )/2 +
(pow(R2,2) - pow(R3,2) )*( Hit3Pos(1) - Hit2Pos(1) )/2/pow( R32,2 ) + 0.5*( Hit2Pos(0) - Hit3Pos(0) )*sqrt( 2 * (pow(R2,2) + pow(R3,2) )/pow( R32,2 )
- pow( pow(R2,2) - pow(R3,2), 2 )/pow( R32, 4 ) - 1 ); Points[3][0] = (Hit2Pos(0) + Hit3Pos(0) )/2 + (pow(R2,2) - pow(R3,2) )*( Hit3Pos(0) -
Hit2Pos(0) )/2/pow( R32,2 ) - 0.5*( Hit3Pos(1) - Hit2Pos(1) )*sqrt( 2 * (pow(R2,2) + pow(R3,2) )/pow( R32,2 ) - pow( pow(R2,2) - pow(R3,2), 2 )/pow(
R32, 4 ) - 1 ); Points[3][1] = (Hit2Pos(1) + Hit3Pos(1) )/2 + (pow(R2,2) - pow(R3,2) )*( Hit3Pos(1) - Hit2Pos(1) )/2/pow( R32,2 ) - 0.5*( Hit2Pos(0) -
Hit3Pos(0) )*sqrt( 2 * (pow(R2,2) + pow(R3,2) )/pow( R32,2 ) - pow( pow(R2,2) - pow(R3,2), 2 )/pow( R32, 4 ) - 1 );

  Points[4][0] = (Hit1Pos(0) + Hit3Pos(0) )/2 + (pow(R3,2) - pow(R1,2) )*( Hit1Pos(0) - Hit3Pos(0) )/2/pow( R13,2 ) + 0.5*( Hit1Pos(1) - Hit3Pos(1)
)*sqrt( 2 * (pow(R3,2) + pow(R1,2) )/pow( R13,2 ) - pow( pow(R3,2) - pow(R1,2), 2 )/pow( R13, 4 ) - 1 ); Points[4][1] = (Hit1Pos(1) + Hit3Pos(1) )/2 +
(pow(R3,2) - pow(R1,2) )*( Hit1Pos(1) - Hit3Pos(1) )/2/pow( R13,2 ) + 0.5*( Hit3Pos(0) - Hit1Pos(0) )*sqrt( 2 * (pow(R3,2) + pow(R1,2) )/pow( R13,2 )
- pow( pow(R3,2) - pow(R1,2), 2 )/pow( R13, 4 ) - 1 ); Points[5][0] = (Hit1Pos(0) + Hit3Pos(0) )/2 + (pow(R3,2) - pow(R1,2) )*( Hit1Pos(0) -
Hit3Pos(0) )/2/pow( R13,2 ) - 0.5*( Hit1Pos(1) - Hit3Pos(1) )*sqrt( 2 * (pow(R3,2) + pow(R1,2) )/pow( R13,2 ) - pow( pow(R3,2) - pow(R1,2), 2 )/pow(
R13, 4 ) - 1 ); Points[5][1] = (Hit1Pos(1) + Hit3Pos(1) )/2 + (pow(R3,2) - pow(R1,2) )*( Hit1Pos(1) - Hit3Pos(1) )/2/pow( R13,2 ) - 0.5*( Hit3Pos(0) -
Hit1Pos(0) )*sqrt( 2 * (pow(R3,2) + pow(R1,2) )/pow( R13,2 ) - pow( pow(R3,2) - pow(R1,2), 2 )/pow( R13, 4 ) - 1 );
...
{
  double distance1 = 0.;
  distance1 = sqrt( pow(Sol1(0) - Gamma1(0),2) + pow(Sol1(1) - Gamma1(1),2) + pow(Sol1(2) - Gamma1(2),2) ) + sqrt( pow(Sol1(0) - Gamma2(0),2) +
pow(Sol1(1) - Gamma2(1),2) + pow(Sol1(2) - Gamma2(2),2) ) + sqrt( pow(Sol1(0) - Gamma3(0),2) + pow(Sol1(1) - Gamma3(1),2) + pow(Sol1(2) - Gamma3(2),2)
); return distance1;
}
