...
JPetRecoSignal TaskC2::createRecoSignal(JPetRawSignal& rawSignal)
{
  ...
  double t1 = 0;
  double t2 = 0;
  if ((trailingPoints.size() == 0u) || (leadingPoints.size() == 0u)) {
    WARNING("Not enough points to calculate TOT from RawSignal");
  } else {

    // on each edge, take the first (smallest) threshold and its corresponding time
    t1 = leadingPoints.front().getValue();
    t2 = trailingPoints.front().getValue();
  ...
  if (rawSignal.getPoints(JPetSigCh::Leading, JPetRawSignal::ByThrValue).size() < 2 ) {
    WARNING("Not enough leading points to calculate time at threshold from RawSignal");
  } else {
