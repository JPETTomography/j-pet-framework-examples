...
bool EventCategorizerImaging::exec()
{
  ...
  if (auto timeWindow = dynamic_cast<const JPetTimeWindow* const>(fEvent)) 
  {
	...
	if (event.getHits().size() > 1) 
	{
		...
		ImagingHits = Imaging( hits, kMinAnnihilationTOT/1000, kMaxAnnihilationTOT/1000, fSaveControlHistos );
		if( ImagingHits )
		{
			JPetEvent newEvent;
...
int EventCategorizerImaging::Imaging( vector<JPetHit> Hits, double MinAnnihilationTOT, double MaxAnnihilationTOT, bool SaveControlHistos )
{
